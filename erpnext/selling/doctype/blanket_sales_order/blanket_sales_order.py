...
class BlanketSalesOrder(SellingController):
	def __init__(self, *args, **kwargs):
		super(BlanketSalesOrder, self).__init__(*args, **kwargs)

	def validate(self):
		super(BlanketSalesOrder, self).validate()
	...
	def validate_ordered_items(self):
		check_list = []
		for d in self.get('items'):
			...
			tot_ordered_qty = frappe.db.sql("select sum() from `tabSales Order` \
				where item_code = %s and warehouse = %s", (d.item_code, d.warehouse))
			#print(d.item_code, d.warehouse, tot_ordered_qty)
	...
	def validate_delivery_date(self):
		if not self.due_date:
			self.due_date = max([d.delivery_date for d in self.get("items")])
